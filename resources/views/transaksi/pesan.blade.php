            ...
            <div class="row">
                @foreach ($dtMenu as $rsMenu)
                    <div class="menu-item col-md-3"
                        onclick="add_menu('{{ $rsMenu->id }}','{{ $rsMenu->mn_nama }}','{{ $rsMenu->mn_harga }}')">
                        <div class="inner">
                            @if ($rsMenu['foto'] != '')
                            ...
                            @endif
                            <h2>{{ $rsMenu->mn_nama }} <br /><span>Rp
                                    {{ number_format($rsMenu->mn_harga, '0', ',', '.') }}
                                    ,-</span></h2>
                        </div>
                    </div>
                @endforeach
        ...
        <div class="col-md-4 pe-0">
            <div class="transaksi">
                <form id="frmTransaksi" action="{{ url('transaction/save') }}" method="post">
                    ...
                    <p>
                        <input style="border: 2px solid rgb(0, 0, 0);" type="text" name="meja" id="meja"
                            placeholder="Meja / Atas Nama">
                    </p>
                    {{-- End Info Transaksi --}}
                    ...
                    <table id="dtCustomer" class="dtTable table table-bordered table-hover">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <button
                                        onclick="pilih_customer('{{ @$rsCus->id }}','{{ @$rsCus->cus_kd }}','{{ @$rsCus->cus_nm }}')"
